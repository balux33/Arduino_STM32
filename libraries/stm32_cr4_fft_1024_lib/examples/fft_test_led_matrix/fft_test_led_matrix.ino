// Real-time audio spectrum analyzer with led matrix
...
CRGB c_lut[16] = {CRGB::Green, CRGB::Green, CRGB::Green, CRGB::Green, CRGB::Yellow, CRGB::Yellow, CRGB::Yellow, CRGB::Orange, CRGB::DarkOrange, CRGB::OrangeRed, CRGB::OrangeRed, CRGB::OrangeRed, \
CRGB::Red, CRGB::Red, CRGB::Red, CRGB::Red};

CRGB ampl_to_color_matrix(float ampl, float max_ampl, uint8_t row)
{
  float step = max_ampl/16;
  if(ampl > step * row)
  {
    return c_lut[row];
  }
 ...
 void draw_col_to_matrix(float ampl, float max_ampl, uint8_t col)
 {
  for (uint8_t i=0;i<16;i++)
  {
    if(col%2 == 0)
    {
       leds[6+16*col+(15-i)] = ampl_to_color_matrix(ampl,max_ampl, i);
    }
    else
    {
       leds[6+16*col+i] = ampl_to_color_matrix(ampl,max_ampl, i);
    }
  }
 }

const uint16_t band_table[] = {0,330,1060,1860,2660,3325,4000,4665,5330,6000,6665,7330,8000,8665,9330,10000,10600};

void print_to_matrix(uint32_t * data)
{
  float ampl_db ;

  const float treshold = 21.0;
  const float max_ampl = 43.0;
  const float max_ampl_first_bin = 47.3;
  ...
  for(uint8_t i=0;i<16;i++)
  {
    ...
    if(i != 0)
      draw_col_to_matrix(ampl_db, max_ampl, i);
    else
      draw_col_to_matrix(ampl_db, max_ampl_first_bin, i);
  }
